...
* All origins in a 1p context can ask whether the user has payment credentials associated with a given payment method. For privacy reasons, browsers are likely to rate-limit this capability, provider users with the option of disabling it, etc. This functionality exists to support the requirement that a "pay" button for a given payment method is displayed only when the user is ready to pay.
