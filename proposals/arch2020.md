# A Vision for Web Payments Architecture: Payment Credentials, Modal Windows, and Streamlined Authentication
...
* Privacy changes in browsers (elimination of third party storage access, less fingerprinting) run the risk of requiring frequent logins across checkouts, and is likely to disrupt current risk assessment techniques used in the payment industry.
...
* to find ways to improve the user experience by "coupling" capabilities more closely in a payments contetx than is possible when using other Web features "out of the box."

Here are some "use cases" that might use these capabilities differently:

|                     | Merchant modal | Payment app modal |  Browser credential selector | Streamlined authentication | 
|---------------------|----------------|-------------------|-----|-----|
| *Pay (one PMI)      |                |         Yes         |     |    | 
...
* "*Pay" referes to digital wallets such as Google Pay, Apple Pay, and Samsung Pay
...
The Chrome implementation of Payment Handler API features a secure modal window that has a number of user experience interesting properties (see previous [blog post](https://www.w3.org/blog/wpwg/2019/10/29/payment-handler-value-proposition/): it keeps the user near the merchant context (instead of a redirect) and the browser displays the origin of the content at the top of the window. Today only Web-based payment apps have access to this secure modal window capability, but we have heard use cases where payment service providers (e.g., embedded in merchant pages) would like the same functionality.

For discussion:

* In light of this architecture, would 1p storage access still be needed in secure modal windows?
...
* An origin would register a payment credential in the browser. This origin is said to "control" the credential. We envision a variety of use cases:
...
* During a transaction, the browser would display the list of registered payment credentials acceptable to the merchant for the transaction.
* The origin that controls the credential can get or update a payment credential from a third party context.

The payment credential controller sets the following properties for each credential:
...
* Unique payment credential identifier. The meaning of this identifier depends on the payment method. The origin that controls the payment credential, for example, might use it to identify a specific bank account or credit card, or might use it to identifier a specific user. Ideally this identifier would be dereferenceable (that is, a URL), but it might not be dereferenced in all contexts (e.g., when used in a back end protocol).

The browser keeps track of which origin controls the payment credential; see below for more information about how the browser might use this information.

When the merchant calls Payment Request API, the browser displays stored payment method credentials that match the payment method identifiers specified by the merchant. When the user selects a credential from the browser's list, the merchant receives the selected payment credential. Note how the payment method information helps determine what happens next:

* Suppose the payment method identifier corresponds to a mobile wallet. The next step might involve invoking code from the origin associated with that wallet and feeding it the payment credential. That code might prompt the user to authenticate (see streamlined authentication below), or open a secure modal window for user interaction.
...
* Can we use the [Credential Management API](https://developer.mozilla.org/en-US/docs/Web/API/Credential_Management_API) for the management of the payment credential?
...
* **Cookie replacement**. On several occasions we have discussed the idea of providing an origin with particular powers when in a known payment context (e.g., in the middle of a Payment Request call). One idea here is to  enable an origin access to the list of payment credentials it controls from a 3p context. We could make payment credentials clearable by the user, but with more user interaction required than generic cookies.
* **Gating the modal window**. The browser could limit the use of the modal window capability to (1) a payment request context and (2) only code from the same origin as the payment credential selected by the user.
* **hasEnrolledInstrument**. Payment Request API has included a <code>hasEnrolledInstrument</code> capability so that merchants (or their PSP) can determine whether the user is "ready to pay" for a particular payment method. This capability is mostly used to make a decision whether to display a button for a particular payment method. Several stakeholders have indicated that this is a valuable method to them, but it does raise some privacy issues. One new idea is that perhaps <code>hasEnrolledInstrument</code> could be limited to an origin learning about the storaged payment credentials that an origin controls.
