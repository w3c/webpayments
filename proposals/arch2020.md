...
Regarding access to these payment credentials:

* The controlling origin can enumerate controlled credentials in either a 1p context or in a payment context within the (3p) in-context display.
* All origins can access the credential selected by the user through Payment Request.
...
* **hasEnrolledInstrument**. Payment Request API has included a <code>hasEnrolledInstrument</code> capability so that merchants (or their PSP) can determine whether the user is "ready to pay" for a particular payment method. This capability is mostly used to make a decision whether to display a button for a particular payment method. Several stakeholders have indicated that this is a valuable method to them, but it does raise some privacy issues.
