...
* All origins can access the credential selected by the user (during Payment Request).
* All origins in a 1p context can ask whether the user has payment credentials associated with a given payment method. For privacy reasons, browsers are likely to rate-limit this capability, provider users with the option of disabling it, etc.
